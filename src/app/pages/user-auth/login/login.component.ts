...
export class LoginComponent implements OnInit, OnDestroy {
  ...
  private getAllSeoPage() {
    const subscription = this.seoPageService.getAllSeoPageByUi({status: 'publish', 'type': 'login-page'}, 1, 1).subscribe({
      next: (res) => {
  ...
  private updateMetaData() {
    ...
    const seoTitle = this.seoPageData?.seoTitle ? this.seoPageData?.seoTitle : 'Login page';
